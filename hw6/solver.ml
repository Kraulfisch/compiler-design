    ...
    let solve (g:Graph.t) : Graph.t =
      ...
      let rec worklist (w:NodeSet.t) (g:Graph.t) : Graph.t =
        if NodeSet.is_empty w then g
          ...
          (* Get predecessors and combine their output facts *)
          let preds = Graph.preds g n in
          let pred_facts = NodeSet.fold (fun p acc -> (Graph.out g p) :: acc) preds [] in
          ...
          if Fact.compare old_out new_out <> 0 then
            ...
            let w'' = NodeSet.fold (fun m acc -> NodeSet.add m acc) succs w' in
            worklist w'' g'
          else
            worklist w' g
      in
