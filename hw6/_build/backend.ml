...
let better_layout (f:Ll.fdecl) (live:liveness) : layout =
  ...
  (* Pre-colored helper function for register parameters *)
  ...
  (* Spill cost heuristic: lower cost = prefer to spill
     Prefer to spill variables live across calls (they need save/restore anyway) *)
  let spill_cost uid =
    let uses = get_use_count uid in
    let deg = degree uid in
    let across_call_penalty = if UidSet.mem uid live_across_call then 0 else 10 in
    if deg = 0 then max_int
    else ((uses * 100) + across_call_penalty) / (deg + 1)
  in
  ...
  (* Simplify: iteratively remove nodes, building a stack for later coloring *)
  let rec simplify (worklist : UidSet.t) (graph : UidSet.t Datastructures.UidM.t) 
                   (stack : uid list) : uid list =
    if UidSet.is_empty worklist then
      ...
      in
      (* Pick which node to remove *)
      let node_to_remove = 
        match low_degree_node with
        | Some uid -> uid
        | None ->
            (* No low-degree node found - pick node with lowest spill cost *)
      ...
      let neighbors = try Datastructures.UidM.find node_to_remove graph with Not_found -> UidSet.empty in
      let graph' = 
        UidSet.fold (fun neighbor g ->
          let n_neighbors = try Datastructures.UidM.find neighbor g with Not_found -> UidSet.empty in
          Datastructures.UidM.add neighbor (UidSet.remove node_to_remove n_neighbors) g
        ) neighbors (Datastructures.UidM.remove node_to_remove graph)
      in
      let worklist' = UidSet.remove node_to_remove worklist in
      simplify worklist' graph' (node_to_remove :: stack)
  in

  (* Select: assign colors by popping from stack (optimistic coloring) *)
  let select (stack : uid list) : (uid * Alloc.loc) list =
  ...
  (* Run the algorithm *)
  let stack = simplify all_uids interference [] in
  let coloring = select stack in
