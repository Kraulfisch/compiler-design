...
module Make (Fact : FACT) (Graph : DFA_GRAPH with type fact := Fact.t) =
  struct
    ...
    (* To compute a fixpoint of the flow on the graph, use this algorithm: *)
    ...
    let solve (g:Graph.t) : Graph.t =
      let module NodeSet = Graph.NodeS in
      let rec worklist (w:NodeSet.t) (g:Graph.t) : Graph.t =
        if NodeSet.is_empty w then g
        else
          let n = NodeSet.choose w in
          let w' = NodeSet.remove n w in

          let preds = Graph.preds g n |> NodeSet.elements in
          let in_fact = List.map (Graph.out g) preds |> Fact.combine in
          ...
          if Fact.compare old_out new_out <> 0 then
            ...
            let succs = Graph.succs g n |> NodeSet.elements in
            let w'' = List.fold_left (fun acc m -> NodeSet.add m acc) w' succs in
            worklist w'' g'
          else
            worklist w' g
      in
