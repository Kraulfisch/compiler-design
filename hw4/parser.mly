...
/* Precedence and associativity - from lowest to highest (spec has highest first) */
...
gexp:
  ...
  | NEW t=ty LBRACKET RBRACKET
    LBRACE es=separated_list(COMMA, gexp) RBRACE
...
exp:
  ...
  | NEW t=ty LBRACKET RBRACKET
    LBRACE es=separated_list(COMMA, exp) RBRACE
                        { loc $startpos $endpos @@ CArr (t, es) }
...
stmt: 
  ...
  | FOR LPAREN vdecls=separated_list(COMMA, vdecl) SEMI
        e=option(exp) SEMI
